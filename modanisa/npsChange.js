...
spApi.modanisaNps.prototype.npsChange = function () {
    var that = this;
    var textArray = {
        tr_TR: 'Mutlu olduğunuzu duyduğumuza çok sevindik! Değerli geri bildirimlerinizle, ' +
            'Trustpilot.com da bize destek olmak ister misiniz?',
        en_US: 'We are pleased to hear that you are happy. ' +
            'Can you please show your support on Trustpilot.com with your valuable feedback?',
        ar_AR: 'سعدنا جداً بنيل رضاكم، هل يمكنكم دعمنا من خلال مشاركة تعليقاتكم القيمة على Trustpilot.com؟',
        ...
        de_DE: 'Wir freuen uns, dass Sie zufrieden sind. ' +
            'Können Sie ebenfalls Ihre wertvolle Rückmeldung auf Trustpilot.com teilen und uns unterstützen?'
    };
    var linkArray = {
    ...
    this.controlLocationFlag = function () {
        var userCountry = JSON.parse(spApi.storageData('userLocation') || '{}').country || '';
        var locationFlag = userCountry === 'US' || userCountry === 'UK';

        spApi.conLog('Location control for npsChange : ', locationFlag);

        if (true) {//burayı degistim
    ...
    this.setNpsValues = function () {
        sQuery('.ins-user-nps-text').text(textArray[spApi.getLang()]);
