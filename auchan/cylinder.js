(function (cylinder) {
    ...
    cylinder.getEndpoint = function () {
        return 'https://recommendation.api.useinsider.com/' + cylinder.configs.partnerId + '/' +
            cylinder.configs.language + '/most/viewed/partner' + '?size=12&details=true';
    };
    ...
    cylinder.populateItems = function (items) {
        ...
        sQuery(items).each(function (index, item) {
            var hasSalePrice = item.price[cylinder.configs.currency] !== item.original_price[cylinder.configs.currency];

            cards +=
                '<div class="ins-product ' + cylinder.configs.classList[index] + ' ' +
                '">' +
                ...
                cylinder.configs.customClass + '0' +'">' +
                ...
                cylinder.getPricesWithSpace(item.original_price[cylinder.configs.currency]) + ' </div>' +
                '<div class="ins-sale-price" ' + (!hasSalePrice && 'style="display: none;"') + '>' +
                cylinder.getPricesWithSpace(item.price[cylinder.configs.currency]) + ' </div>' + '</div>' +
                '<div class="ins-cart-elements">' +
                '<div class="ins-go-to ' + cylinder.configs.customClass + '0' + '">' +
                '<a href="' + item.url + '">>' + cylinder.configs.buttonText + '</a>' +
                '</div>' + '</div>' + '</div>' + '</div>' + '</div>';
        });
    ...
    cylinder.getPricesWithSpace = function (price) {
